...
Affinitysql =""" select cmb.product , cmb.rel_product ,cmb.num_basket,
                    ...
                    WHERE td.tran_dt < current_date() AND td.tran_dt >= date_sub(current_date(), INTERVAL 12 MONTH)
                    GROUP BY td.product_id, rel_td.product_id) cmb
                    join
                    (select product_id , count(*) as num_basket 
                    from tran_dtl td 
                    WHERE tran_dt < current_date() AND tran_dt >= date_sub(current_date(), INTERVAL 12 MONTH)
